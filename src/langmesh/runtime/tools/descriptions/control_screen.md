...
**A workflow can be a file.** `screen` is an instance of the importable `langmesh.screen.Screen`, so the same calls work in a saved module as they do inline. The shape is ordinary Python, and nothing about it is special to any one task:
