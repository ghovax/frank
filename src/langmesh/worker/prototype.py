...
logger = logging.getLogger("langmesh.prototype")
...
def _settle_proxy_environment() -> dict[str, str]:
    ...
    # Frozen, `sys.executable` is the `langmesh` binary rather than an interpreter, so the packaged build answers a role instead of `-c`.
...
def session_command(assignment_fd: int, ready_fd: int, lifeline_fd: int) -> list[str]:
    ...
    return [sys.executable, "-m", "langmesh", "session", *numbers]
...
def _load_runtime() -> None:
    ...
    import langmesh.worker.serve  # noqa: F401
    import langmesh.worker.server  # noqa: F401
    ...
    def _become_session(
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                logging.getLogger("langmesh.worker").exception("Session process could not exec")
            return 1
