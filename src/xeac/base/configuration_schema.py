...
`xeac configure` used to be able to show only what a file already contained, which is exactly
...
:class:`~xeac.base.tuning.Default`, and this module walks the models to produce the list. Both
...
def _tuning_defaults(prefix: str) -> list[Setting]:
    ...
    they are the names in :class:`~xeac.base.tuning.Tunable`, each of which already carries its
