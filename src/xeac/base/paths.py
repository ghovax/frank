"""Where XEAC keeps things on disk, following the XDG Base Directory convention.
...
DAEMON_SOCKET_FILENAME = "xeacd.sock"
...
def config_directory() -> Path:
    """User-editable configuration (``~/.config/xeac``)."""
...
def state_directory() -> Path:
    """Logs and pidfiles (``~/.local/state/xeac``)."""
    return _xdg("XDG_STATE_HOME", Path.home() / ".local" / "state")


def cache_directory() -> Path:
    """Regenerable caches, safe to delete at any time (``~/.cache/xeac``)."""
