...
Broadcaster and context event bus in :mod:`xeac.daemon.state`."""
...
def _publish_stream_event(context_id: str, part) -> None:
    ...
    state.event_bus.publish(context_id, part.model_dump(by_alias=True, exclude_none=True, mode="json"))
...
def _set_turn_state(context_id: str, running: bool) -> None:
    ...
    if not running and updated == 0:
        state.event_bus.complete(context_id)
