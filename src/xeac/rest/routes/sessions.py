...
async def session_tasks(context_id: str):
    ...
    tasks = await state.task_store.tasks_for_context(context_id)
    return {
        "tasks": [
...
async def session_task_page(context_id: str, before_row_id: int | None = None, limit: int = 400):
    ...
    """
    assert state.task_store is not None
    page = await state.task_store.task_page_for_context(context_id, before_row_id=before_row_id, limit=limit)
    return {
        "tasks": [
            task.model_dump(by_alias=True, exclude_none=True, mode="json")
...
async def delete_session(context_id: str):
    ...
    if state.task_store is not None:
        ...
        for text in await state.task_store.context_message_texts(context_id):
        ...
        await state.task_store.delete_context(context_id)
        for path_string in referenced_uploads:
            if not await state.task_store.any_history_references(path_string):
