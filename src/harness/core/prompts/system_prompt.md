...
- **Prefer functional code and existing library functions over hand-rolled ones.** Reach for the language's built-ins, the standard library, and the framework's primitives before writing your own loop, parser, or helper. The library version is tested, documented, and already handles the edge cases you will forget — so search the documentation first, then build around what the library already gives you instead of reinventing it.
