...
- **Background results arrive on their own — finish, and you'll be woken.** A started `bash`/`web_search` task returns a handle, not a result. When the remaining work depends on a pending result, end your turn: the harness starts a fresh turn on its own the moment the result lands and re-engages you with it in context — you don't block, poll, or keep the turn busy. Never `read_task` a `bg-…`/`search-…` handle.
