...
class AgentPathSegment(BaseModel):
...
# The location of the agent that produced an event. Empty = the root/main agent.
AgentPath = list[AgentPathSegment]
...
#
# Every event is a DataPart payload of the shape {kind, path, ...}. The client
# discriminates on `kind`. `path` places the event in the agent tree.

class _EventBase(BaseModel):
    path: AgentPath = Field(default_factory=list)
...
class GroupStartedEvent(_EventBase):
    ...
    agent_name: str
    title: str = ""
    # The parent tool-call (the spawn_agent card) this group belongs to, so the UI can
    # link the transcript card to the panel group.
    tool_call_id: str = ""
...
class TokenUsageEvent(_EventBase):
    ...
    # Combined spend of the agents this agent spawned — a separate bucket.
    agents: AgentUsage = Field(default_factory=AgentUsage)
...
MODEL_ENVELOPE_MODELS: tuple[type[BaseModel], ...] = (
    ModelToolResult, TurnContext, ToolMetadata, AgentPathSegment,
)
