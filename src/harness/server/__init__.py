...
:mod:`harness.server.boot` owns the runtime — the FastAPI ``app``, its shared singleton
state, the ORM models, and the operations the routes call — and imports no route module.
:mod:`harness.server.asgi` is the thin ASGI entry that mounts the split routers onto that
