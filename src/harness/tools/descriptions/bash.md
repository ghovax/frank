...
- *Fetch a URL:* use **fetch_url** (not `curl`/`wget` for reading a page)
- *Download a file:* use **download_file** (not `curl -O`/`wget` for saving bytes to disk)
...
`location` defaults to the local filesystem; pass it only to run the command on a different (remote) location. A command on a remote location behaves exactly like a local one — same output shape, backgrounding, and safety contract — it just executes on that host in the location's directory.

**Safety contract** — treat the `read_only` flag as part of the contract, not decoration:
- `read_only=true` for commands that only **inspect state** (`pwd`, `ls`, `rg`, `cat`, `sed -n`, `git diff`, `git status`). Set it **explicitly** — an omitted flag defaults to `false`, and the command is treated as potentially mutating.
- `read_only=false` (the default) for anything that **modifies** files, processes, caches, databases, or external state. Set `risk` accordingly:
