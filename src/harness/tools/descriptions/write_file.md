...
- If the file already exists, you **must** call **read_file** first — this tool errors otherwise.
- **Always prefer editing existing files.** Never write new files unless explicitly required.
- **Never** proactively create documentation files (`*.md`, README) unless the user explicitly asks.
- Use this instead of `bash` with `echo >`, `cat <<EOF`, or `sed`/`awk` for creating or fully replacing a file. For *targeted* changes to an existing file, prefer **replace_lines**.
