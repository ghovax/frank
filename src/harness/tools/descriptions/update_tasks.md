...
For each update, move the task to `in_progress` when you start it, `completed` when it is **actually done**, and `blocked` when reality diverges from the plan.
...
- **Never** end the turn with steps still unresolved that you in fact completed — reconcile first.
