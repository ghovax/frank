...
- File contents come back with each line prefixed as `<line>: <content>`. Use those lines to build unified diff hunks for **apply_patch**; do not include the `<line>:` prefixes in patch text.
- File reads include a content hash used internally to reject stale edits if the file changes before **apply_patch** or **write_file** runs.
