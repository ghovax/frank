...
**Usage:**
- You **must** call **read_lines** before editing. The runtime records the file hash from that read and rejects the edit if the file changed afterward.
- Line numbers are **1-indexed** and refer to the `<line>:` prefixes shown by `read_lines`.
...
- After a successful edit, call `read_lines` again before the next `replace_lines` call for the same file, because line numbers may have shifted.
