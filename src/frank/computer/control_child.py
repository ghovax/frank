"""The child process that runs a `control_screen` script, holding no Frank code and no surface state."""
...
def _load_screen_module(package_root: str):
    """`frank.screen` executed on its own, without the package body it lives under, and registered under both names."""
...
def _script_namespace(
    ...
    # `frank.screen` loaded from its file, so the package's `__init__` never drags the whole runtime in.
