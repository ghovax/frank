{{ app }} is displaying a window ({{ width }}x{{ height }}) but does not publish it to macOS accessibility, so there are no controls to read or act on. This is the app's own choice and nothing here can change it while it runs: both accessibility handshakes were tried and refused.
