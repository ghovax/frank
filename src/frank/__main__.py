...
def main(argv: list[str] | None = None) -> int:
    ...
    # The daemon, the prototype and a session are internal entry points, addressed by a leading
    # word. All are stripped before the rest is handed on, so none sees it as a subcommand.
    ...
    # One session worker, exec'd by the prototype. Its assignment — session id, tokens, paths,
    # confinement profile — arrives on an inherited pipe rather than in `argv`, because `argv`
    # is world-readable through `ps` and the assignment carries this session's capability token
    # and the daemon's.
    ...
    # `frankd`, not `daemon`: the CLI has its own `daemon` verb for inspecting and starting the
    # service, and routing that here would mean `frank daemon status` silently tried to start a
