...
async def upload_file(file: UploadFile = File(...)):
    """Store a user file under Frank's managed home, content-addressed, and return its generic metadata."""
...
async def reference_attachment(reference: AttachmentReference):
    ...
    # One builder, shared with `frank.Session`, so the two front doors cannot describe a file differently.
    try:
