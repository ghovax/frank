...
async def list_terminals(session_id: str = "", working_directory: str = ""):
    ...
    # A freshly opened terminal is live before it has persisted any scrollback; surface it too so its tab does not vanish on a mid-session refresh.
...
async def terminal_websocket(
    ...
    try:
        ...
        if remote_alias and location_base_directory.strip():
            # Remote terminal: the directory is the remote base dir (used verbatim in the ssh `cd`, never touched locally), and the session sshes to the host alias.
        ...
        # Logged, and that is not decoration.
