...
async def full_disk_access_status():
    """Whether the daemon process can read Full-Disk-Access-protected data (Screen Time, Safari history)."""
...
async def accessibility_status():
    """Whether the daemon can control other apps (read the AX tree, synthesize input) — the permission the computer-use tool needs."""
...
async def list_models_endpoint():
    """The model catalog for the picker: every known model with its provider and whether its provider has a resolvable credential (available), plus the provider registry."""
...
async def recent_models():
    """Recently selected models (newest first), mirroring the project history — a user can quickly switch back to a model they used before without scrolling the full catalog."""
    return {"models": await asyncio.to_thread(_recent_models)}


@router.get("/auth/chatgpt")
async def chatgpt_auth_status():
    """Whether a ChatGPT subscription is signed in, and for which account — so the settings dialog and the model picker can show sign-in state and unlock the ``chatgpt`` provider's models."""
...
async def chatgpt_auth_start():
    """Begin an OAuth sign-in: bind the loopback callback server and return the authorize URL for the client to open in a browser."""
...
async def chatgpt_auth_signout():
    """Sign out: clear the stored tokens and reset runtimes so the ``chatgpt`` provider re-locks immediately."""
...
async def cursor_auth_status():
    """Whether a Cursor subscription is signed in, and for which account — so the settings dialog and the model picker can show sign-in state and unlock the ``cursor`` provider's models."""
...
async def cursor_auth_start():
    """Begin a Cursor sign-in: return the login URL for the client to open in a browser and start polling for its completion in the background."""
...
async def cursor_auth_signout():
    """Sign out: clear the stored tokens and reset runtimes so the ``cursor`` provider re-locks immediately."""
...
async def get_settings():
    """Return the API credentials stored in the XDG configuration file so the settings dialog can pre-fill them, including per-provider keys."""
    assert state.global_configuration is not None
...
async def update_settings(request: SettingsUpdateRequest):
    """Persist API credentials to the XDG configuration file and apply them live: refresh the in-memory configuration, the Exa client, restart the MCP client manager so Composio tools appear/disappear with its key, and drop cached agent runtimes so the next turn rebuilds with the new credentials."""
    assert state.global_configuration is not None
...
async def reset_setting(path: str):
    """Put one setting back to what the code ships, by removing it from the file."""
    from frank.base import configuration_file
    from frank.base.configuration_schema import setting_for
...
async def update_user_context(request: UserContextUpdateRequest):
    """Persist and apply the opt-in user-context toggle."""
    assert state.global_configuration is not None
    async with state.configuration_lock:
...
async def update_computer_control(request: ComputerControlUpdateRequest):
    """Persist and apply the opt-in computer-use toggle."""
    assert state.global_configuration is not None
    async with state.configuration_lock:
...
async def update_toolbox(request: ToolboxUpdateRequest):
    """Persist and apply whether sessions may install tools for themselves."""
    assert state.global_configuration is not None
    async with state.configuration_lock:
...
async def update_compaction(request: CompactionUpdateRequest):
    """Persist and apply the Observational-Memory compaction settings (the auto on/off trigger and thresholds), independently from credentials."""
    assert state.global_configuration is not None
