...
async def delete_session(session_id: str):
    ...
    # Settle any input-required pause and drop the awaiting-input marker; the pending record and tasks are removed with the session below regardless.
    await state.session_deleted(session_id)
    # Delete every task in the context from the task store, then reclaim any upload files the session referenced that no surviving session still references (uploads are content-addressed and may be shared, so only truly-orphaned files are removed).
    if state.turn_store is not None:
        ...
        # One call drops the context's tasks (head/history/artifacts) and its conversation checkpoint — the single durable turn surface.
    ...
    # The retention pass above already removed persisted conversation and lifecycle state; finish by removing the sidebar record.
    def _delete_record() -> bool:
        ...
        try:
            ...
            if record is None:
                ...
                return False
            # A workspace pointing at a conversation that no longer exists would send the next client that opened it looking for a session nothing can serve, so the pointer is cleared here rather than being left for a reader to discover.
