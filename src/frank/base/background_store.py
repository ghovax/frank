"""Durable record of background jobs, so an interrupted long-running task can survive a server restart."""
...
from frank.base.paths import background_database_path as _background_database_path

if TYPE_CHECKING:  # pragma: no cover - typing only
    from frank.base.ports import JobStore
from frank.base.sqlite_lock import background_sqlite_write_lock, configure_background_sqlite_lock
...
class BackgroundJobStore:
    ...
    def record_process_group(self, job_id: str, process_group: int) -> None:
        """Record the OS process-group id of a job's shell subtree once it has started, so :func:`reap_orphaned_processes` can kill it after a crash."""
        with background_sqlite_write_lock():
            with self._connect() as connection:
                connection.execute(
    ...
    def orphaned_process_groups(self) -> list[int]:
        """Process groups of jobs still marked running from a previous, unclean shutdown — the orphans a reaper must kill on startup."""
        with self._connect() as connection:
    ...
    def undelivered_jobs(self, session_id: str, agent_name: str) -> list[dict[str, Any]]:
        """A context's jobs that carry a result the model has not yet seen — whether they completed normally or were abandoned by a restart."""
    ...
    def contexts_with_undelivered(self, agent_name: str) -> list[str]:
        """Distinct contexts of an agent that have any deliverable-but-undelivered result — the ones to wake on startup."""
        with self._connect() as connection:
            rows = connection.execute(
