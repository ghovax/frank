...
def _decode_jwt_claims(token: str) -> dict:
    """Decode a JWT payload without verifying the signature — we read only the expiry and the subject for display, and trust neither for authorization."""
    try:
...
def _expiry_of(access_token: str) -> float:
    """The access token's own expiry, or an hour out when the token is not a readable JWT — a short guess is safer than a long one, because guessing long means calls fail before a refresh is attempted."""
...
def _tokens_from_payload(payload: dict, previous: Optional[CursorTokens] = None) -> CursorTokens:
    """Build a :class:`CursorTokens` from a poll or refresh response, carrying the previous refresh token over when the response does not return a usable one."""
