...
def _shell_arguments(values: list[str]) -> Optional[dict[str, Any]]:
    """``ShellArgs{command, working_directory}`` becomes ``bash``."""
...
def _write_arguments(values: list[str]) -> Optional[dict[str, Any]]:
    """``WriteArgs{path, file_text}`` becomes ``write_file``."""
...
def _list_arguments(values: list[str]) -> Optional[dict[str, Any]]:
    """``LsArgs{path}`` becomes ``bash``, because listing a directory is a command and the harness has no separate tool for it."""
...
def _background_shell_arguments(values: list[str]) -> Optional[dict[str, Any]]:
    """``BackgroundShellSpawnArgs{command, working_directory}`` becomes ``bash`` in the background, which is the same tool the harness already has for exactly this."""
...
def _list_resources_arguments(values: list[str]) -> Optional[dict[str, Any]]:
    """``ListMcpResourcesExecArgs{server}`` becomes ``list_mcp_resources``, whose parameter is spelled the same because both are naming the same thing."""
...
class ChatCursorModel(BaseChatModel):
    ...
    def _environment(self) -> bytes:
        """The ``RequestContextEnv`` a turn describes itself with — where the client believes it is running, and what it is running on."""
    ...
    def _auth_error(status: int, detail: str) -> Exception:
        """The exception an HTTP failure becomes, chosen so the caller can tell whether trying another backend could possibly help."""
...
def clear_resumptions() -> None:
    """Forget every resumable conversation."""
    _resumptions.clear()
