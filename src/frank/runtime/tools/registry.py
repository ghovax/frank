...
async def control_screen(
    ...
    Two of them are worth knowing before you need them. `shortcuts()` on a window returns the application's own menu bar as a tree, with the chord each item advertises, spelled the way `press` takes it — **never press a shortcut you remembered**, because a remembered chord belongs to some version of some application you are not looking at, and asking costs one call for an authoritative answer. `evaluate` on a page runs JavaScript and returns its result natively, so you can filter or aggregate in the page and bring back only what you need, or call the site's own signed-in API with fetch through the user's real session.
