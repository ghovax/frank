...
async def control_screen(
    ...
    Acting — call by bare name, no prefix. On BOTH surfaces:
      - click(element, button="left", count=1)
      - type(element, text, submit=False, mode="replace") — fills a field, and with submit=True presses Return after
      - press(key) — a named key, a letter, or a chord like "cmd+shift+g"
      - focus(element=None) — give keyboard focus to this place, or to one control in it. It never raises a window over what the user is doing; input reaches a window that is minimized, behind others, or on another desktop, because every event goes to the process rather than to the screen
      - scroll(element=None, direction="down")
      - drag(element, onto)
      - read(element=None) — the lines this place says, or one element's, as a list
      - select(element, text=None, to_text=None, select_all=False) — highlight a substring, a range from the caret up to some text, or the whole field
      - caret(element, before=None, after=None, at_offset=None, edge="") — place the cursor before/after some text, at a character offset, or at the "start"/"end" edge

    Windows also have:
      - shortcuts() — the application's own menu bar as a tree, with the chord each item advertises, spelled the way press() takes it. **Never press a shortcut you remembered.** A remembered chord belongs to some version of some application you are not looking at; asking costs one call and the application's answer is authoritative. Guessing cost a task four attempts and changed a pane nobody asked to change.
    ...
    Browser targets also have — these names are simply absent when the target is a window, so a NameError tells you at once rather than after the plan is committed:
      - hover(element)
      - choose(element, option)
      - upload(element, paths)
