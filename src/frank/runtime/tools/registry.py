...
async def control_screen(
    ...
    **This is a program, not a pair of clicks.** The single most common mistake with this tool is writing three timid lines and paying for another round trip to learn what the fourth should be. You have loops, conditionals, comprehensions, `try`/`except`, and the standard library for computing over what a find returned — use them. A find that raises is catchable, so branch on it. `wait_for(query, seconds=...)` blocks until something matches, which is how you say "once the pane has loaded" instead of hoping; `sleep(seconds)` is there for the rest. A script that walks a list, decides per item, waits for what each action reveals, and reports a summary at the end is one call. The same work as separate calls is a dozen, and each one starts blind because nothing carries between them but element ids. Reach as far as the task actually goes.
