...
async def control_screen(
    ...
    Finding elements:
      find_many(query, limit=8, all=False, role="", name="", context="") — the ranked matches, for reading or harvesting a whole set (act on every result). find_one(query, role="", name="", context="") — the single best match, for acting; it returns that one element, or raises if the top matches are indistinguishable. Both take the same three narrowing arguments, and reaching for them is usually better than lengthening the query: ranking happens *inside* the narrowed set, so role="button" searches only buttons rather than hoping buttons float to the top. Prefer them whenever you know the kind of control you want — the ranking is by wording alone and carries no notion of what a thing *is*, so "the search button" competes against every element that mentions search, while query="search", role="button" does not. Write explicit, descriptive queries — name the target and the section it sits under; a terse query spreads across every similar control. On the browser, find also searches the page's own traffic (network requests and WebSocket frames), so you can pull data from the source instead of walking the DOM.
