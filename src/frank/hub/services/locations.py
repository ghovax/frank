"""Location domain: the location/workspace serialization and session-location resolution primitives shared by the workspaces service."""

from __future__ import annotations

from datetime import datetime, timezone
from frank.locations.resolver import host_is_defined, location_uri_for, LocationAddress
from frank.protocol.dtos import LocationInput
...
def _serialize_location(record: LocationRecord) -> dict[str, Any]:
    """A location for the API: its generated URI (identity), derived name, connection, and its one execution policy (permission_mode)."""
    try:
...
def _derive_location_name(database_session, workspace_id: str, kind: str, base_directory: str, host_alias: str, *, exclude_id: str = "") -> str:
    """The agent-facing name for a location, derived from its connection (not user-entered): the SSH host alias for a remote, the base directory's folder name for a local (falling back to "local"/"remote")."""
    if kind == "remote":
...
def _location_pair_conflict(first: tuple[str, str, str], second: tuple[str, str, str]) -> str | None:
    """The overlap message for a single pair of normalized (machine, path, raw) locations, or ``None`` if they don't conflict."""
...
def _locations_conflict_message(entries: list[tuple[str, str, str]]) -> str | None:
    """A human message for the first pair of locations that overlap on the same machine — identical base directories, or one nested inside another — which is redundant and ambiguous for the agent to address."""
    normalized = [
        (
...
def _workspace_id_for_location(location_id: str) -> str:
    """Which workspace a location belongs to, or ``""`` if there is no such row."""
...
def _resolve_session_locations(session_id: str) -> list[dict[str, Any]] | None:
    """The runtime-shaped locations for a session's workspace: each entry carries the generated URI and the *effective* execution settings (own value, else workspace default)."""
    if state.session_factory is None:
        return None
    database_session = state.session_factory()
    try:
