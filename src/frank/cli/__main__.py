"""`frank`, the command: the same surface the desktop client uses, with verbs that mirror the API exactly."""
...
def _command_serve(arguments: argparse.Namespace) -> int:
    """Make Frank available: a control plane and the interface in front of it, as one verb because neither was separately useful."""
...
def _command_reach(arguments: argparse.Namespace) -> int:
    """Make Frank reachable from somewhere that is not this machine, which `serve`'s loopback proxy cannot cover."""
