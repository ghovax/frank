...
from daisy.base.paths import configuration_file_path, database_file_path  # noqa: F401 — re-exported
...
# XDG convention and is resolved in `daisy.base.paths`.
...
class Section(BaseModel):
    ...
    to work and no longer does — is accepted, written back, listed by `daisy configure`, and
...
class SandboxConfiguration(Section):
    ...
    def to_profile(self):
        """This configuration as the :class:`daisy.base.confinement.Profile` the spawn path
...
class TuningConfiguration(Section):
    ...
    defaults: dict[str, float] = Field(
        ...
        description=(
            "Override one tunable by its own name, in its own unit — see `daisy configure --all`. "
    ...
    def _known_defaults(cls, value: dict[str, float]) -> dict[str, float]:
        from daisy.base.tuning import unknown_tunable_names
...
class AgentDefaults(Section):
    ...
    permission_mode: Literal["default", "auto", "read_only"] = Field(
        ...
        description=(
            ...
            "side effect. A default, not a ceiling: `daisy create --mode` overrides it, and a "
...
class GlobalConfiguration(Section):
    ...
    providers: dict[str, ProviderCredential] = Field(
        ...
        description=(
            ...
            "openrouter, xai and the rest; daisy.base.providers holds the full list with each "
