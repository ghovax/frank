...
``chatgpt.com/backend-api/codex/responses`` (see :mod:`daisy.runtime.models.codex`).
...
async def valid_tokens() -> ChatGPTTokens:
    ...
    if tokens is None:
        raise ChatGPTAuthError(
            "Not signed in to ChatGPT. Run `daisy auth login`, sign in from Settings, or "
            "drive `daisy.base.credentials.ChatGPTLoginFlow` yourself."
        )
