...
def configuration_file_path() -> Path:
    return daisy_home_directory() / CONFIGURATION_FILENAME


def database_file_path() -> Path:
    return daisy_home_directory() / DATABASE_FILENAME
...
class TuningConfiguration(BaseModel):
    ...
    tool applies is derived from the *live* model context window (see ``daisy.core.tuning``), so a
