...
`daisy` command and the desktop client call, reached directly instead of through a shell.

Doing it through the shell was the earlier design, and it could not hold. A session that runs
`daisy create` writes argv as free text, and free text cannot carry what makes a peer a *peer*:
