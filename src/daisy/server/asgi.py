...
:mod:`daisy.server.boot` builds the FastAPI ``app`` and drives its lifespan; this module
...
services, never this entry. Import target for a server: ``daisy.server.asgi:app``.
"""
...
from daisy.server.boot import app
...
# e.g. `from daisy.core import artifact_versioning as artifacts` — and a bare
...
for _route_name in (
    ...
    app.include_router(importlib.import_module(f"daisy.server.routes.{_route_name}").router)
