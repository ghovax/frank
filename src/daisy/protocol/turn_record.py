...
(``daisyTurnKind`` beside an unprefixed ``pendingInteraction``) named the owner twice in one
...
# it means "these are Daisy's attributes"; it does not belong in the field names underneath.
from daisy.protocol.metadata import DAISY_METADATA_KEY

TURN_STATE_KEY = DAISY_METADATA_KEY
...
class ToolGate(BaseModel):
    ...
    twin of the in-process :class:`~daisy.runtime.turn_events.SuspensionGate`, so a suspend
