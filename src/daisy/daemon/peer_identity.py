...
detached, so a server that shells out to `daisy` calls as nobody. It is worth being plain about
the shape of that: a process outside the session's process session is also outside what
`daisy kill` signals and outside the tree the reaper walks. It has stopped being the session
