...
def _set_turn_state(session_id: str, running: bool, retains: bool = False) -> None:
    ...
    if (previous == 0) != (updated == 0):
        ...
        # indistinguishable from a model thinking — so `daisy wait` had nothing to wait for
...
async def _session_claim_work_habits(params: dict) -> dict:
    ...
    from daisy.workspace.services.sessions import claim_work_habits_acknowledgement
...
async def _session_title(params: dict) -> dict:
    ...
    from daisy.workspace.services.sessions import _set_session_title
