...
from daisy.protocol.dtos import ArtifactAnnotationSaveRequest

from datetime import datetime
from datetime import timezone
from fastapi import HTTPException
from daisy.daemon.persistence import versioning as artifacts
...
from daisy.locations.resolver import executor_for
...
from daisy.daemon.persistence.database import ArtifactAnnotationRecord, ArtifactFileRecord, ArtifactSurfaceRecord, ArtifactVersionRecord, SessionLifecycleRecord, SessionRecord
from daisy.daemon.services.broadcast import _publish_broadcast
from daisy.daemon.services.locations import _resolve_session_locations
...
def _artifact_maximum_bytes() -> int:
    ...
    workspace = getattr(state.global_configuration, "workspace", None) if state.global_configuration else None
...
def _capture_artifacts(
    *, session_id: str, location_uri: str, executor: LocationExecutor, base_directory: str,
    ...
    if state.main_loop is not None and state.main_loop.is_running():
        state.main_loop.call_soon_threadsafe(state.capture_queue.put_nowait, request)
    else:
        try:
...
async def _capture_worker() -> None:
    assert state.capture_queue is not None
    while True:
        request = await state.capture_queue.get()
        try:
