...
export type GlyphName =
  | "globe" | "terminal" | "mouse-pointer-click" | "download" | "message-circle-question" | "target"
...
const NEVER_MUTATES: ReadonlySet<string> = new Set([
  "search_web", "fetch_url", "read_turn", "wait_for",
...
function assertDistinctGlyphs(): void {
  ...
  if (seen.has("wrench")) {
    throw new Error('"wrench" stands for a tool with no glyph of its own; a real tool may not wear it.');
  }
}
...
export function toolCallDisplay(name: string, args: Record<string, unknown> | undefined): ToolDisplay {
  return {
