...
Each session serves [A2A](https://github.com/google/A2A) (JSON-RPC) on **its own unix socket** in the runtime directory. Anything holding that address can talk to it: you from the terminal, the desktop app, or another session. There is no in-process delegation and no `spawn_agent` tool — an agent that needs a peer creates a session and speaks the same protocol to it that a person does. A child appears in `xeac ps`, can be attached to, and is reaped when its parent ends.
