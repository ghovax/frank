...
- **Right-click** `Frank.app`, choose **Open**, then **Open** again in the dialog, or
- from a terminal:
...
| 5 | `packaging/build-daemon.sh` | Freezes the harness, then smoke-tests it in an isolated set of XDG directories | `freezing the harness…`, then `ok: frankd answers on its own socket`, then the install commands it prints | **several minutes** |
...
| Two `frank` on your `PATH` behave differently | The checkout's and the installed one share `~/.config/frank/` and the runtime directory, so whichever daemon started first owns it | `which -a frank`, and check `image.executable` in `frank daemon status` |
