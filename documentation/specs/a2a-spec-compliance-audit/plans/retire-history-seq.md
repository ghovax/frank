...
- **Append.** Insert `(task_id, message)` and let the database assign `row_id` — no `MAX`, no computed position, no unique-position constraint, so two appends can never collide on a position. The store still reads a **plain `COUNT(*)`** (`_history_count`, kept but reduced to a count) to slice the suffix of the only-ever-growing `task.history` not yet stored; that count determines *which* messages are new, never *where* they go. The alternative — a stateless idempotent insert keyed on `message_id` that drops the count entirely — was rejected because it conflicts with compaction's positional in-place rewrite below (which maps compacted message *i* onto the *i*-th existing row): a natural-key insert would order strictly by insert time and could not place a reordered compaction. So the count stays, correct under the same write lock that serializes every other store write; what is gone is the hand-computed *position* and the collision it caused.
...
One focused, self-contained change to `task_store.py`: drop the `seq` column + unique constraint and set `sqlite_autoincrement`; reduce `_history_count` to a plain `COUNT(*)` (a suffix count, no longer a position); make `save`'s non-terminal append a bare `(task_id, message)` insert; repoint every `ORDER BY seq` to `row_id`; swap the latest-per-task `MAX(seq)` for `MAX(row_id)` in the paginated context read; and rewrite `_compact_persisted_history` to order and tail-delete by `row_id`. It lands in one step because the column cannot be half-removed.
...
Landed as one change to `task_store.py` and verified against the fake-LLM harness (not committed): a task's history appends preserve order; a burst of concurrent saves of one task never collides, never duplicates, and reconstructs to the correct sequence; a terminal-compaction rewrite stays ordered and duplicate-free; and a real executor tool turn round-trips its persisted history through `save`/`get`/the context read. The one deviation from the plan is documented above — the append keeps a plain `COUNT(*)` (`_history_count`) to slice the unsent suffix rather than dropping it entirely; a `message_id`-keyed idempotent insert would have removed it but could not honor compaction's positional in-place rewrite, so the count stays while the hand-computed position and its collision are gone.
