...
This plan collapses the two mechanisms Daisy grew for one idea — a turn that pauses and resumes — into a single durable primitive. [`input-required.md`](./input-required.md) made a *top-level* turn durable: it suspends as a segment, persists a checkpoint, and resumes from the database across a restart. [`delegated-agent-permissions.md`](./delegated-agent-permissions.md) made a *delegated* turn pause too, but by a different route — it parks on an in-memory future and resumes in place, ephemerally. Two pause models, two persistence stories, one concept. This plan unifies the **durable state** the two shared but stored apart: the model-facing conversation, the restart policy, and the resolve/overlay surfaces all move behind one source of truth — the A2A task store. A top-level turn is the durable primitive (suspend → persist checkpoint → rebuild → resume). The delegated pause keeps its in-process parked continuation, but it is now a documented, justified specialization of the one model rather than a divergent implementation — the reasoning is in the delegation section. There is no backward-compatibility constraint: the context-keyed `conversations` table is removed, not kept beside the checkpoint. (Two collapses the first draft proposed — converting delegation to durable segments, and folding `background_store` into a task inbox — were evaluated during implementation and deliberately not done; the sections below say why.) It builds on the findings in [`audit.md`](../audit.md).
...
## Suspend and resume — top-level durable, delegated parked

For a **top-level** turn, suspension is the durable segment: the runtime's preflight (the shared decision path) yields the pending interactions, the executor writes them to the task's control state and closes the segment as A2A `input-required` with `final=True`, and the answer — an external `input_response` `message/send` or a native REST resolve — rebuilds the runtime from the task's checkpoint and drives `resume_stream` as the next segment. Restart policy: preserve.

For a **delegated** turn, the pause stays an **in-process parked continuation** — the model established in [`delegated-agent-permissions.md`](./delegated-agent-permissions.md), unchanged here. This is a deliberate specialization, and the reasoning matters because the plan's first draft proposed converting it to a durable segment:

- **A durable segment would buy nothing.** A delegated pause is ephemeral on restart (the decision in `delegated-agent-permissions.md`): its parent's live driver is gone after a restart, so the child is failed regardless. Writing a durable checkpoint for it only to discard it on reload is pure waste.
- **The reason segments exist does not apply to delegation.** Segments were forced for top-level turns because a spec-compliant external client answers with a *new* `message/send`, and parking the coroutine across that let the SDK tear down the still-in-use queue. A delegated turn's "client" is the parent's in-process driver consuming the delegate stream; parking the child coroutine keeps its queue producer alive (verified against the real SDK earlier), so there is no queue-teardown to avoid.
- **The user-facing behavior is already unified.** The resolve path (`resolve_pending_input`) and the approval overlay already handle a parked delegated gate through the same surfaces as a top-level pause; the divergence that remains is purely internal (an in-memory future versus a durable checkpoint).

Converting a working, correct, already-user-unified parked model into a durable-segment reimplementation — for no durability and no user-visible gain, at real regression risk — is not a good trade even with unlimited effort. So the delegated pause keeps its parked continuation, and "unification" here means the shared *checkpoint*, *reconciliation*, *resolver*, and *overlay* — with the delegated in-process continuation as a documented, justified specialization for ephemeral in-process work.
...
No compatibility shim; the superseded surface is removed:
...
Deliberately **kept** (see their sections for why): `_agent_permission_futures` and the parked delegated continuation; `background_store`; and `PENDING_INTERACTION_KEY` (the pending-interaction record stays keyed under it in the task metadata — it is already on the task, so there is nothing to consolidate).
...
The delegated pause (parked continuation) and `background_store` are kept, for the reasons in their sections.

## Implementation status

Steps 1–5 are implemented and committed, each a coherent, compiling increment. Unit-verified against real SQLite / imports: the checkpoint save/load and compaction-replace, the reconciliation policy matrix (top-level and unmarked preserved; delegated and mid-execution failed; terminal untouched), the inbox lifecycle before it was removed, and `delete_context`. Not verifiable in this environment (no live model or running server): the end-to-end top-level suspend→persist→rebuild→resume across a real turn, and the safe-point `CHECKPOINT` write cadence under a real tool loop — these compile and are wired correctly by inspection but need a live run, as with every durable-turn change. The delegated-segment rework and background-store fold from the original draft were evaluated and deliberately not done, for the reasons documented above (no durability or user-visible gain, real regression risk against working code).

## Testing

The checkpoint round-trips through `messages_to_dict`/`messages_from_dict` and a compaction rewrite replaces the snapshot rather than corrupting it (verified). The reconciliation policy matrix is unit-tested against real SQLite (verified). Still to exercise on a live run: a top-level turn suspending as a `final=True` `input-required` segment and resuming from the checkpoint to completion; crash-consistency by cutting the process at a `CHECKPOINT` safe point and asserting the resumed/next turn is at-most-once (a tool whose `ToolMessage` was not written is reported interrupted and never re-run, no side effect duplicated); and that a reopened long session rehydrates its conversation from the checkpoint.
