...
## The last two questions, settled

**The placeholder is worth reading, and the coverage number is the wrong reason.** `AXPlaceholderValue` reaches 5 elements in 1,146 — 0.44%, which on the rule this document keeps applying would make it worthless. It is kept anyway, because of *which* five: `Search` in System Settings, `Search` and `Filter` in Skim, `Go to file/function` in RStudio, `Describe what to build` in Code. Every one is a search or filter field, which is the single most likely thing a query goes looking for, and every one would otherwise be called "text field" — the system's prose for its kind, shared with every other field on screen. It fires only where there is no name, so it can cost nothing; a rarely-used fallback that is exactly right when it does fire is a different proposition from a field added to every key. That is a judgement rather than a measurement, and it is recorded as one: with five instances there is nothing to bootstrap.

**The tie-break is not worth building.** Ranking elements that have a real name above elements keyed by their fallback, whenever two scores sit within 0.02 of each other, changes the outcome of **zero** queries out of 1,259 — a difference of +0.0% with an interval of [+0.0%, +0.0%]. The 4.2% that the value fallback costs on named elements is not recoverable this way, because the losses are not near-ties; the fallback text wins those queries outright rather than edging them. The machinery is not built, and the cost stands as the honest price of making two thirds of a window reachable.
