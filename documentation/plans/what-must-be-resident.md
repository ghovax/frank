...
| # | Change | Where | Why |
|---|---|---|---|
| 27 | ~~Split the database into `history.db` and `workspace.db`~~ — **not taken, and the reason is #19** | — | Recorded rather than silently skipped. The split's purpose was to let the browser surface be served by a process that is not the daemon, and #28 achieves that on its own: `rest` no longer imports `daemon` at all. What changed in between is that **#19 merged the two session records into one table**, so `sessions` is now read by the sidebar *and* owned by the registry. Splitting the files would have to either duplicate that table or read across two databases — re-separating exactly what #19 just merged, for a benefit #28 already delivers. Two writers to one file is what `base/sqlite_lock.py`'s cross-process `fcntl` lock has always been for, and `background.db` has worked that way all along |
...
| 35 | ~~`sqlite_lock` learns about a third database~~ — **not needed**, since #27 was not taken | `base/sqlite_lock.py` | It knows two, and two is now the right number. The reason it distinguishes them is unchanged and still load-bearing: the async task store holds the history lock across an `await`, so a synchronous acquire of the same lock on the loop thread would deadlock the process |
...
| **This fired.** `litellm` builds a module-level `httpx.Client()` at import; `httpx.Client()` calls `urllib.request.getproxies()`; on macOS that is `getproxies_environment() or getproxies_macosx_sysconf()`, and the second reaches SystemConfiguration through `_scproxy`, leaving **two permanent native threads**. The prototype went from one thread to three, refused to fork, and the children that did fork aborted on `+[__NSPlaceholderSet initialize]` — the multi-threaded-fork abort, not the CoreFoundation verdict | Found on macOS by the prototype's own `task_threads() == 1` refusal, exactly as designed, and attributed to the precise import by the `macos-fork` stage. **The checker could not have caught it**: it forbids network calls at import inside `src/`, and this was inside a third-party package. Fixed by `_settle_proxy_environment()` in `worker/prototype.py`, which resolves the real proxy configuration in a throwaway subprocess and exports it *before* anything imports `litellm`, so `getproxies_environment()` short-circuits and `_scproxy` is never reached |
| **A unix socket path exceeds `sun_path`** | `sun_path` is 104 bytes on macOS and 108 on Linux, so a path that binds in development can be unbindable on the platform the product ships to. A session socket was **117 bytes** on macOS — no session could bind at all — while every Linux run stayed green. `bind` reports `AF_UNIX path too long` and names neither the path nor the limit | The `socket-paths` stage checks the *class*: every socket the harness can construct, under a modelled macOS `$TMPDIR`, against the smaller limit. `base/paths.py` raises `SocketPathTooLong` at construction with the path and its length. The socket is named by a digest of the session id rather than the id itself, which takes the macOS path to 93 bytes |
