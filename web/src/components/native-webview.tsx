...
function boundsChanged(a: PreviewBounds | null, b: PreviewBounds): boolean {
  if (!a) return true;
  return (
...
function visibleBounds(element: HTMLElement): PreviewBounds | null {
...
// Embeds the native preview webview over this placeholder. It measures its own rect
...
export function NativeWebview({ url }: { url: string }) {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (shownForUrlRef.current !== url) {
        ...
        void nativePreviewShow(url, bounds);
        return;
      }
      if (boundsChanged(lastBoundsRef.current, bounds)) {
        lastBoundsRef.current = bounds;
        void nativePreviewSetBounds(bounds);
      }
    };
  ...
  return (
    <Box ref={holeRef} position="relative" w="100%" h="100%" minH="320px" bg="bg.subtle" overflow="hidden">
      ...
      <Flex position="absolute" inset={0} align="center" justify="center" direction="column" gap={2} color="fg.subtle">
        ...
        <Flex align="center" gap={2}>
          ...
          <Text fontSize="xs">Loading preview…</Text>
        </Flex>
      </Flex>
    </Box>
  );
