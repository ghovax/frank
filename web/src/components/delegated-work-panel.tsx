...
// A session and everything it created.
...
function buildSessionTree(entries: SessionEntry[]): SessionTreeNode[] {
  ...
  for (const node of nodes.values()) {
    // A child whose parent is not in this list (living in another workspace, or deleted out from under it) is promoted to a root rather than dropped — a session is never unreachable because of where its parent happens to be.
    const parent = node.entry.parentSessionId ? nodes.get(node.entry.parentSessionId) : undefined;
    if (parent && parent !== node) parent.children.push(node);
    else roots.push(node);
  }
  // Children oldest first, so a fan-out reads down the page in the order it happened — the only order in which a delegation is legible.
  for (const node of nodes.values()) {
...
export function DelegatedWorkPanel({
  ...
  // Every session the client knows about, in the sidebar's order.
  sessions: SessionEntry[];
  // The conversation this panel is about: the sidebar's selection.
  ...
  const root = useMemo(() => {
    ...
    // A delegated session opened directly is not a root of the whole forest, so find it wherever it sits: the panel stays scoped to the conversation, not to the row clicked.
