...
export function DelegatedWorkPanel({
  ...
  // What the selected conversation delegated, as the tree it forms. Its own row is not here:
  // it is the row already lit in the sidebar, and repeating it would spend the panel's first
  // line saying what you just clicked.
  const groups = useMemo(() => {
    if (!rootSessionId) return [];
    const root = buildSessionTree(sessions).find((node) => node.entry.sessionId === rootSessionId)
      // A delegated session opened directly is not a root of the whole forest, so find it
      // wherever it sits: the panel stays scoped to the conversation, not to the row clicked.
      ?? findNode(buildSessionTree(sessions), rootSessionId);
