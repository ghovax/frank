...
// The chat-history sidebar as a self-contained unit: the workspaces list, a new-session row, and each workspace's sorted sessions (status dot + marquee title + options menu), one flat list.
...
// A session row, its status vocabulary and its hover card all live in `session-row.tsx`, shared with the session-tree panel so a conversation reads the same in both.

// The workspace hover card follows the Git bar's shape — a titled heading with the glyph that stands for the thing, then label/value rows — because that is already the vocabulary this interface uses for "here is what I know about this", and a second one would only make the two harder to read.
...
// Row geometry, shared by the New-session row, the session rows, and the footer so their leading glyphs, text, and left edge all line up on one grid — the reference sidebar's single-column rhythm.
const ROW_MINIMUM_H = "30px";
// Just wide enough to hold the 14px row glyph / 8px status dot centered — kept tight so titles hug the left edge of the pill rather than floating in from a wide empty gutter.
...
export function SessionsSidebar({
  ...
  // The profiles a schedule can be given.
  agents: AgentSummary[];
}) {
  const translation = useTranslations("SessionsSidebar");
  ...
  const visibleWorkspaces = workspaces
    .map((workspace) => ({
      workspace,
      // Only the conversations *you* started.
    ...
    <PanelCard flex={1}>
      ...
      <PanelBody pt={1}>
            ...
            <DropdownMenu
              trigger={
                <Button
                  ...
                  gap={1}
                  size="2xs"
                  // The menu returns focus to this trigger on close, which fires a stray focus-visible ring even after a pointer selection.
        ...
        {!sessionsLoaded || workspaces.length === 0 ? null : visibleWorkspaces.length === 0 ? (
          // The same alert the settings panel uses, rather than a line of muted text.
