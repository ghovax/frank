...
function zoneOptions(): ZoneOption[] {
  ...
  return known
    ...
    // The machine's own zone first, because it is the answer nine times in ten and scrolling to it alphabetically is absurd.
    .sort((left, right) => {
...
export function TimezoneSelect({
  ...
  // Built once: 444 zones each formatted for their current offset is real work, and the offsets do not move while a dialog is open.
  const options = useMemo(() => zoneOptions(), []);
  // `contains` rather than `startsWith`: people search for the city, not the region, so `Rome` finds Rome and `Africa` finds the continent.
  ...
  const { collection, filter } = useListCollection<ZoneOption>({
    ...
    filter: contains,
    // 444 rows would be mounted on every keystroke otherwise.
    limit: 50,
  });
