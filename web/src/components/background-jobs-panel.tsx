...
// A shell command surfaced from the transcript, carried in the exact shape the ToolCall component consumes so each row renders as a real tool call.
interface ShellJob {
  toolCallId: string;
  name: string;
  arguments: Record<string, unknown>;
  // Absent when the event carried none.
  status?: ToolEventStatus;
  result: unknown;
  timestamp: string;
  running: boolean;
  canBackground: boolean;
  // Already detached (the model ran it with background=true, or the user pushed a foreground command to the background).
...
// A running shell command: the tool card plus the actions that only make sense while it is live — pushing a still-blocking foreground command to the background, or stopping it outright.
...
export function BackgroundJobsPanel({
  ...
  // The set of terminals for this session's context, and which one is on top.
  ...
  // A terminal's environment is chosen when it's created (via the "＋" menu when there is more than one location) and fixed for its life — a shell on host A and a shell on host B are simply different terminals, so the location is not changed mid-session.
