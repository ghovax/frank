...
export class Outbox {
  ...
  add(message: OutboxMessage): void {
    ...
    this.announce();
    // Not while a decision is outstanding: the answer is known, and asking again would produce a second refusal and a second thing to tell them about.
  ...
  private async pump(): Promise<void> {
    ...
    try {
      while (this.messages.length > 0) {
        // Asked before each message rather than once: answering one decision can leave another open, and the second message must not be thrown at a session that is parked again.
        if (this.ports.parked()) {
          this.hold("decision");
          return;
        }
        const head = this.messages[0];
        // Marked before the await, so a view never draws it as waiting.
  ...
  private hold(reason: OutboxHold): void {
    // Always announced, even when the reason is the one already showing.
    this.held = reason;
    this.announce();
  }
}
