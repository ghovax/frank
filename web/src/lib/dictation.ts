...
// Where the processor is served from.
...
export interface Dictation {
  // Everything said so far, transcribed, and the microphone released.
  stop: () => Promise<string>;
...
export interface DictationRecording {
  // Everything captured so far, as one contiguous buffer, and everything released.
...
async function startDictationRecording(): Promise<DictationRecording> {
  if (typeof navigator === "undefined" || !navigator.mediaDevices?.getUserMedia) {
    // Naming the cause, because the two are fixed in completely different places.
    const insecure = typeof window !== "undefined" && !window.isSecureContext;
    throw new DictationRecordingError(insecure ? "dictationInsecureOrigin" : "dictationNoMicrophone");
  }
  let stream: MediaStream;
  try {
    // No processing options are requested.
    stream = await navigator.mediaDevices.getUserMedia({ audio: true });
  } catch (caught) {
    // The one failure worth naming precisely: permission, which the person can fix, versus anything else, which they cannot.
  ...
  return {
    async stop() {
      ...
      settled = true;
      // One tick before tearing the graph down, so the last block the audio thread posted is delivered rather than dropped with the port.
