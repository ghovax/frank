...
// A Chakra Flex that is also a motion component, so the history sidebar can animate its open/close (opacity + slide) without losing its flex-layout props.
...
// SessionEntry and the sessions-sidebar UI live in the SessionsSidebar component (the chat history is its own unit); this page owns the data + the notification tracking.

// The last workspace the user was in, remembered so a fresh launch reopens it (there is no landing page to pick from) — and the last conversation within it, which the daemon keeps on the workspace record itself.


// The conversation a session belongs to: itself if you started it, otherwise the one at the top of the chain that created it.
...
// A session that is actually working.
...
function Workspace() {
  ...
  // Where this interface opens and what it looks like, as the daemon has it.
  ...
  // The app opens straight into a workspace workspace — there is no landing page.
  const workspaceId = searchParams.get("workspace") ?? "";

  // After the user grants Accessibility and the app relaunches, turn computer control on automatically once.
  useEffect(() => {
  ...
  // Which conversation the daemon last saw this workspace opened at: `null` until it has said, then an id or `""` for none.
  ...
  useEffect(() => {
    ...
    listWorkspaces()
      .then((workspaces) => {
        ...
        // Nothing to open into — release the restore below, which is waiting on a workspace that is never going to arrive, so it can settle on the empty composer rather than on nothing.
    ...
    return () => { cancelled = true; };
    // `readLastWorkspace`/`writeLastWorkspace` close over the preferences and are rebuilt every render; naming them here would re-run this on every render rather than when the workspace it is resolving actually changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [workspaceId, router]);

  useEffect(() => {
    // No workspace yet means the effect above is still choosing one; it will set this itself if it turns out there is none to choose.
    if (!workspaceId) return;
    let cancelled = false;
    getWorkspace(workspaceId)
      .then((workspace) => { if (!cancelled) setRememberedSession(workspace?.last_session_id ?? ""); })
      .catch((caught) => {
        // A workspace that would not load is not a reason to hang on the loading screen; the empty composer is a fine place to land.
  ...
  // Sidebar notifications: sessions that finished a run while you weren't viewing them (an "unread" completion), so the dot means "there is something new here".
  ...
  // Keep the active-session id readable from callbacks (loadSessions) without adding it to their deps and thrashing them on every session switch.
  useEffect(() => {
    activeSessionIdRef.current = activeSessionId;
  }, [activeSessionId]);
  // `?settings=<section>` (from the Workspaces home cog) opens the workspace with that Settings section already showing.
  ...
  // The working directory is derived from the workspace's first local location (for the workspace/agent-resolution that still keys off a path).
  ...
  // The right-hand panels, held here rather than inside ChatPanel because that component is remounted on every conversation switch.
  const [openSidePanels, setOpenSidePanels] = useState<SidePanelKey[]>([]);
  // Default right-region width: comfortable for one panel without dwarfing the transcript (pairs with the sidebar default below).
  const [sidePanelWidth, setSidePanelWidth] = useState(480);
  // Default sidebar width: enough for typical session titles without eating the transcript.
  ...
  // Agents, their cards (skills), MCP servers and memories are all scoped to the selected folder — home globals plus that folder's own `.agents`, deduped, never the server's launch directory.
  ...
  const loadAgents = useCallback(() => {
    fetchAgents(workingDirectoryRef.current)
      .then((agentList) => {
        setAgents(agentList);
        // Keep the current selection when the folder still offers it, otherwise take the first agent on offer.
  ...
  const loadSessions = useCallback(async () => {
    ...
    // One daemon, one fetch.
    let merged: SessionEntry[];
    try {
      merged = mapSessions(await fetchSessions());
    } catch (caught) {
      // A transient failure (a slow probe right after a send, while the server is busy) keeps what we already have rather than blanking the list.
      swallowed({ component: "workspace-page", operation: "list the sessions" }, caught);
      return;
    }
    // Structural sharing: reuse the previous object for any session whose data is identical, so an equal refetch yields identity-stable rows and the list does not re-render or flash.
    ...
    // Flag any non-active session that just went from busy to idle (finished a run while you weren't looking) as an unread completion.
    ...
    // A background session newly waiting on a decision gets the same attention cue the active session's overlay plays — the yellow dot alone is easy to miss.
  ...
  // Coalesce the burst of sessions_changed events a single turn emits (running true, title generated, message saved, running false) into one trailing refetch, so the session list settles once instead of re-rendering three or four times in quick succession.
  ...
  useEffect(() => {
    ...
    // Arm the audio cues on the first user interaction (browsers keep audio suspended until a gesture); every later chime plays immediately.
    primeSounds();
    loadSessions()
      .catch(() => loadSessions());
    loadSettings();
    ...
    // Home is the default workspace for a brand-new chat; the restoration effect below applies it (or the active session's own folder) — we don't force it here, or it would clobber a session opened directly via ?session=.
    fetchHomeDirectory()
      .then(setHomeWorkspace)
      .catch((caught) => swallowed({ component: "workspace-page", operation: "read the settings" }, caught));

    // Live reload: refresh agents when they change on disk, and the session list when a session's (LLM-generated) title is updated.
    const unsubscribe = subscribeEvents((event) => {
      if (event.type === "agents_changed") {
        loadAgents();
        loadAgentCards();
        // A manual edit to an agent's config file (e.g. its permission mode or model) also drives Settings — refetch it so the dialog reflects the on-disk change.
        loadSettings();
      }
  ...
  // Reload the agents and their cards whenever the selected folder changes (and on first render): the available agents, skills and MCP servers are all path-scoped, so picking a different workspace must re-derive what's actually available there rather than showing the launch directory's capabilities.
  useEffect(() => {
    loadAgents();
    loadAgentCards();
  }, [workingDirectory, loadAgents, loadAgentCards]);

  // The working directory is bound to the active context: an open session is restored to its own persisted folder, a brand-new chat falls back to home.
  ...
  if (restoredContext !== contextKey) {
    if (activeSessionId) {
      ...
      // A brand-new chat inherits the working directory the user was just in — no jarring folder reset when starting a new conversation.
      setRestoredContext(contextKey);
      if (!workingDirectory) {
  ...
  // A session used to have to wait for its backend to be activated before it could be opened.
  ...
  // The composer draft belongs to the session, not to the registry listing, so it is read from its own endpoint when a session is opened.
  const [activeSessionDraft, setActiveSessionDraft] = useState("");
  // Cleared while rendering rather than in the effect below, which is React's own answer to "reset state when something changes": an effect runs after the paint, so opening a session would show the previous session's draft in its composer for a frame first.
  ...
  useEffect(() => {
    ...
    // `handleResumeSession` is redefined every render and is not a dependency anything wants to re-run on; the ref above is what bounds this to once.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sessionsLoaded, sessions, workspaceId, activeSessionId, rememberedSession]);

  // Sidebar sort: "recent" (newest first, the load order) or "active" (sessions needing attention or running float to the top, then newest).
  ...
  // Switch the active workspace from its sidebar row: remember it, start a fresh chat in it, and swap the `?workspace=` param (which re-derives the session list, agents, and working directory for that workspace).
  ...
  // Open a workspace's real Settings dialog from its sidebar menu.
  ...
  async function handleDeleteSession(sessionId: string) {
    ...
    if (ok) {
      refreshSessions();
      // Only reset the open conversation when it's the one being deleted; deleting a background session from the sidebar must not disturb what you're looking at.
  ...
  async function handleResumeSession(entry: SessionEntry) {
    ...
    // The restoration effect rebinds the working directory to this session's own persisted folder; no need to set (or re-record) it here.
  ...
  // Opening a session from the delegated-work panel. Deliberately not `handleResumeSession`.
  ...
  // Keep the native menu-bar tray's recent list in sync, and let its "New Chat" and recent-conversation entries drive the app (desktop only).
  ...
  // The active agent's configured model identifier (provider/model), shown on the composer chip and used for attachment/vision gating.
  const agentModel = agents.find((agent) => agent.id === selectedAgent)?.model ?? "";

  function handleAgentChange(agentName: string) {
    // Switching persona continues the current conversation — the new agent picks up the same session (its system prompt is injected on top of the shared history).
    setSelectedAgent(agentName);
  }

  // The composer's model chip reconfigures the active agent's model through the same agent-config endpoint Settings uses.
  async function handleAgentModelChange(modelIdentifier: string) {
    ...
    try {
      ...
      // The model the user picked did not save.
  ...
  // Derive the working directory from the workspace's first local location (the workspace/agent-resolution still keys off a path).
  useEffect(() => {
    if (!workspaceId) return;
    let cancelled = false;
  ...
  return (
    // Floating-panel shell: the chat is the base surface — plain white (bg) that fills the whole window; only the SIDE panels are elevated cards.
    <Flex
      h="100dvh"
      minW={0}
      bg="bg"
      ...
      <AnimatePresence initial={false}>
        {historyOpen && (
          <MotionFlex
            ...
            ml={{ md: 2 }}
            mb={{ md: 2 }}
            // On a phone this panel *is* the screen, so its bottom edge is the device's — the last conversation in the list would otherwise sit under the home indicator.
            pb={{ base: "var(--safe-bottom, 0px)", md: 0 }}
            // `100%` rather than `100dvh`: the parent has already reserved the top inset, so a full-viewport child overflows the screen by exactly that much.
            h={{ base: "100%", md: "auto" }}
            flexShrink={0}
            position="relative"
            minH={0}
