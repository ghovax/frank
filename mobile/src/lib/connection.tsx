...
/** Secrets go to the keychain, not to AsyncStorage — these are bearer tokens with full control of somebody's laptop. */
const store = {
  async read(): Promise<Pairing[]> {
    ...
    try {
      ...
      // Unreadable is the same as absent: there is nothing to salvage from a corrupt token, and the way out — pair again — is the same either way.
      return [];
    }
  },
...
export function ConnectionProvider({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback(async (machine: Pairing) => {
    ...
    // Configured before the status changes, so nothing renders as "online" with the API still pointing at the previous machine.
  ...
  // A phone that has been in a pocket has had its streams dropped and possibly its network changed.
  ...
  const add = useCallback(async (next: Pairing) => {
    // Keyed on the address, so pairing the same machine again replaces its token rather than leaving a second entry holding one that no longer works.
