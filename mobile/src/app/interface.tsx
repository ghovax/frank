...
export default function InterfaceScreen() {
  ...
  // Back to the list when there is nothing to show.
  useEffect(() => {
  ...
  return (
    // No padding here on purpose.
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      <WebView
        ref={view}
        source={{ uri: source }}
        style={{ flex: 1, backgroundColor: theme.colors.bg }}
        // Deliberately *not* `allowsBackForwardNavigationGestures`.
        mediaCapturePermissionGrantType="grant"
        allowsInlineMediaPlayback
        mediaPlaybackRequiresUserAction={false}
        // The token became a session cookie; without this it is dropped between loads and the interface would ask to be paired again on every launch.
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        // Replacing rather than opening: a browser would treat a new window as a popup, and there is nothing on this screen worth going back to.
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[
        styles.centre,
        {
          // Longhand, and the order matters: a `padding` written after `paddingTop` overrides it, which had been quietly throwing the safe-area reservation away.
          paddingTop: insets.top + theme.space[6],
          paddingBottom: insets.bottom + theme.space[6],
          paddingHorizontal: theme.space[6],
          gap: theme.space[4],
