...
2. **Add a model key.** Open **Settings → Providers**, paste a key for any provider (or sign in with a ChatGPT or Cursor subscription), and pick a model. Keys live in your Daisy configuration file — see the [Configuration guide](documentation/configuration.md), or run `daisy configure --all` to see every setting there is.
