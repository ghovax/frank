...
# sign local Frank builds. A stable signing identity (vs. per-build ad-hoc) gives the
...
# deleting "Frank Local Codesign" from Keychain Access.
set -euo pipefail
...
openssl req -x509 -newkey rsa:2048 -nodes -days 3650 \
  -keyout "$WORK/frank.key" -out "$WORK/frank.crt" -config "$WORK/cert.conf" 2>/dev/null
...
/usr/bin/openssl pkcs12 -export -inkey "$WORK/frank.key" -in "$WORK/frank.crt" \
  -out "$WORK/frank.p12" -name "$IDENTITY" -passout pass:frank 2>/dev/null

# Import into the login keychain and let codesign (and any tool, -A) use the key without
# an interactive keychain-access prompt.
security import "$WORK/frank.p12" -k "$KEYCHAIN" -P frank -T /usr/bin/codesign -A
