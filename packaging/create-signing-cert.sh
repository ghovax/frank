...
# sign local Daisy builds. A stable signing identity (vs. per-build ad-hoc) gives the
...
# deleting "Daisy Local Codesign" from Keychain Access.
set -euo pipefail
...
openssl req -x509 -newkey rsa:2048 -nodes -days 3650 \
  -keyout "$WORK/daisy.key" -out "$WORK/daisy.crt" -config "$WORK/cert.conf" 2>/dev/null
...
/usr/bin/openssl pkcs12 -export -inkey "$WORK/daisy.key" -in "$WORK/daisy.crt" \
  -out "$WORK/daisy.p12" -name "$IDENTITY" -passout pass:daisy 2>/dev/null

# Import into the login keychain and let codesign (and any tool, -A) use the key without
# an interactive keychain-access prompt.
security import "$WORK/daisy.p12" -k "$KEYCHAIN" -P daisy -T /usr/bin/codesign -A
