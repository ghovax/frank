...
- Use GitHub's **[Report a vulnerability](https://github.com/ghovax/frank/security/advisories/new)** (Security, then Advisories) to open a private advisory, **or**
...
- **`frank reach` is the one surface meant to leave the machine, and it is opt-in.** Nothing binds past loopback — including `frank reach` itself, which listens on `127.0.0.1` and has no flag that changes that. What carries it off the machine is `tailscale serve`, which terminates TLS with a certificate issued for your machine's `*.ts.net` name and proxies to that loopback port. So the transport is WireGuard between your own authenticated devices, no port is open on the LAN and nothing is forwarded at a router. On top of that it holds a long-lived token in `~/.local/share/frank/reach-token` (`0600`) and refuses every request — HTTP and websocket — that does not present it. `tailscale funnel`, which would put the same listener on the public internet, is deliberately not used: this is a bearer token with full control of the machine, and the sentence above still applies to it. `frank reach rotate` invalidates every paired device. See [`frank reach`](documentation/cli.md#reaching-it-from-a-phone).
