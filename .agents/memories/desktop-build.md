...
macOS caches `AXIsProcessTrusted` per process, so after granting, the **daemon must restart** to see the grant (its workers are re-execs of it). Restarting the *app* no longer does that — the daemon is not its child. The Settings dialog asks the daemon to restart itself over the control plane (`daemon.restart`, also `daisy daemon restart`), which **ends live sessions** because shutdown reaps every one, then calls the `restart_app` Tauri command to reload the webview against the fresh daemon and auto-enables computer-control on return (localStorage `daisy:pendingComputerControlEnable`).
